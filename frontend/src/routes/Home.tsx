          ...
          >
            <div>
              ...
              <div className="text-xs mt-1">
                ...
                {item.reserved_by && (
                  <span className="ml-2 opacity-70">• Rezervisao: {item.reserved_by}</span>
                )}
                {item.purchased_by && (
                  <span className="ml-2 opacity-70">• Kupio: {item.purchased_by}</span>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-2">
              {item.status === 'available' && (
                <button
                  onClick={() =>
                    mustHaveName(() =>
                      reserve.mutate({ id: item.id, name, note })
                    )
                  }
              ...
              {item.status !== 'purchased' && (
                <button
                  onClick={() =>
                    mustHaveName(() =>
                      purchase.mutate({ id: item.id, name, note })
                    )
                  }
